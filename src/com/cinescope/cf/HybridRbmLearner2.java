 ...
 */
public class HybridRbmLearner2 {
    
    /**
     * Creates a new instance of CDStochasticRBMLearner
     */
    public HybridRbmLearner2() {
    }
    
    public static double Learn(RBM userBasedRbm, RBM userBasedRbm2, RBM itemBasedRbm, double[][] data, double[][] transposedData) {
    	// user-based
    	...
    	// user-based 2
        double[][] userHiddenActivities2 = userBasedRbm2.getHiddenActivitiesFromVisibleData(data);
        ...
        // user-based 2 negative phase
        double[][] userNegPhaseVisible2 = userBasedRbm2.getVisibleActivitiesFromHiddenData(userHiddenData2);
        // null values missing in original data
        RbmBasedCF.nullMissingValues(data, userNegPhaseVisible2);
        ...
        for (int i=0; i<negPhaseVisible.length; i++) {
        	for (int j=0; j<negPhaseVisible[i].length; j++) {
        		negPhaseVisible[i][j] = (userNegPhaseVisible[i][j] + itemNegPhaseVisible[j][i] + userNegPhaseVisible2[i][j]) / 3.0;
        	}
        }
        ...
        double[][] userNegPhaseHidden2 = userBasedRbm2.getHiddenActivitiesFromVisibleData(negPhaseVisible);
        ...
        // update weights for user-based 2
        double[][] userWeightUpdates2 = getConnectionWeightUpdates(userBasedRbm2, data, userHiddenActivities2, negPhaseVisible, userNegPhaseHidden2);
        updateWeights(userBasedRbm2, userWeightUpdates2);
        userBasedRbm2.UpdateHiddenBiases(userHiddenActivities2, userNegPhaseHidden2);
        userBasedRbm2.UpdateVisibleBiases(data, negPhaseVisible);
        ...
        userHiddenActivities2 = userBasedRbm2.getHiddenActivitiesFromVisibleData(data);
        userNegPhaseVisible2 = userBasedRbm2.getVisibleActivitiesFromHiddenData(userHiddenActivities2);
        // null values missing in original data
        RbmBasedCF.nullMissingValues(data, userNegPhaseVisible2);
